...
FVector UMyTools::MyUnclampLerp(FVector A, FVector B, float T)
{
...
{
	/*FVector Result = MyUnclampLerp(A, B, T);

	Result.X = MyClamp(Result.X, 0, T);
	Result.Y = MyClamp(Result.Y, 0, T);
	Result.Z = MyClamp(Result.Z, 0, T);

	return Result;*/

	T = UMyTools::MyClamp(T, 0, 1);
	return UMyTools::MyUnclampLerp(A, B, T);
}
...
FVector UMyTools::BezierInterp(const TArray<FVector>& Points, float Alpha)
{
	...
	{
		//Onpasse dans chaque point en s'arretant a l'avant dernier
		//Car on utilise 2 points par tours de boucle (i et i+1)
		for (int i = 0; i < InitialLerpPoints.Num() - 1; i++)
		{
			//On effectue le lerp entre les deux point et on stock le résultat
